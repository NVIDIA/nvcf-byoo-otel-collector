...
docker build --secret id=SERVICE_USER_ACCESS_TOKEN,env=SERVICE_USER_ACCESS_TOKEN \
  --build-arg OTEL_BUILDER_VERSION=v0.143.0 \
...
3. **YAML validation** - Use `make validate-otelconfig` to verify generated YAML is valid
4. **Embedded configs** - Templates are embedded at build time
...
7. **Secrets handling** - Secrets are extracted to individual files, ensure proper file permissions
...
- Go module: `github.com/NVIDIA/nvcf/monitoring/byoo-otel-collector`
